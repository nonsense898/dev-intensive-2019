...
object Utils {
    fun parseFullName(fullName: String?): Pair<String?, String?> {
        ...
        val firstName: String? = parts?.getOrNull(0)?.ifEmpty { null }
        val lastName: String? = parts?.getOrNull(1)?.ifEmpty { null }
        ...
        payload.trim().forEach {
            nickname += when {
                ...
                dictionary.containsKey(it.toLowerCase()) ->
                    if(it.isUpperCase()) dictionary[it.toLowerCase()]?.capitalize(Locale.ROOT) else dictionary[it]
                else -> it
            }
        }
        return nickname
